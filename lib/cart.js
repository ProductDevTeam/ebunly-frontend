...
// There is no bulk-clear endpoint, so clearCart removes each line individually.
...
// ── Clear the whole cart — no bulk endpoint, so remove each line ─────────────
export async function clearCart(itemIds = []) {
  await Promise.all(itemIds.map((id) => removeCartItem(id)));
}
