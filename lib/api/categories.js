...
/*
 * The unscoped shelf: everything, filtered only by whatever is in the query
 * string. This is where search results and the cross-category filters live now
 * that /discover is gone — it needs a slug because the results surface is
 * /shop/categories/[id], and it deliberately has no subcategories so the page
 * renders the product list rather than a type grid.
 */
export const ALL_CATEGORY_SLUG = "all";

const ALL_CATEGORY = {
  id: ALL_CATEGORY_SLUG,
  label: "All Gifts",
  description: "Every gift on Ebunly — filter by occasion, recipient or budget",
  subcategories: [],
};
...
const RECIPIENT_CATEGORIES = {
  "for-her": {
    ...
    label: "For Her",
    description:
      "Jewellery, beauty, fashion and more — curated gifts she will love",
    subcategories: [
      {
        id: 1,
        slug: "jewellery",
        name: "Jewellery",
        desc: "Necklaces, Bracelets & more",
  ...
  "for-him": {
    ...
    label: "For Him",
    description:
      "Stylish and thoughtful gifts for the special man in your life",
    subcategories: [
      {
        id: 1,
        slug: "men-fashion",
        ...
        image: "/categories/men-fashion.png",
      },
      {
        id: 2,
        slug: "watches",
        name: "Watches",
        desc: "Smartwatches, Luxury & more",
      ...
      },
      {
        ...
        slug: "wallets",
        name: "Wallets",
        desc: "Wallets, Card Holders & more",
        image: "/categories/wallets.png",
      },
      {
        id: 4,
        slug: "footwear",
        name: "Footwear",
        desc: "Sneakers, Loafers & more",
        image: "/categories/footwear.png",
      },
    ],
  },
  "for-couples": {
    ...
    label: "For Couples",
    description:
      "Gifts they will both love — perfect for anniversaries, weddings and celebrations",
    subcategories: [
      {
        id: 1,
        slug: "gift-boxes",
        name: "Gift Boxes",
        desc: "Curated sets for two",
        image: "/categories/bags.png",
      },
      {
        id: 2,
  ...
  "for-babies": {
    ...
    label: "For Babies",
    description:
      "Sweet and memorable gifts for newborns, toddlers and little ones",
    subcategories: [
      {
        id: 1,
        slug: "newborn-gifts",
        name: "Newborn Gifts",
        desc: "Clothing, Blankets & more",
...
//  Occasions (homepage featured categories)
const getOccasionsData = unstable_cache(
  async () => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}/categories?featuredOnHomepage=true&limit=20`;
    const res = await fetch(url, { next: { revalidate: 60, tags: ['occasions'] } });
    if (!res.ok) return [];
  ...
  ['occasions'],
  { revalidate: 60, tags: ['occasions'] }
);

export async function getOccasions() {
  return getOccasionsData();
}
...
export async function getCategoryBySlug(slug) {
  if (slug === ALL_CATEGORY_SLUG) {
    const { subcategories: _ignored, ...meta } = ALL_CATEGORY;
    return meta;
  }
  ...
  return {
    ...
    linkedOccasionTags: cat.linkedOccasionTags ?? [],
  };
}
...
export async function getSubcategoriesBySlug(slug) {
  ...
  if (RECIPIENT_CATEGORIES[slug]) {
    ...
    console.log(
      `[subcategories] "${slug}" — using hardcoded data (${subs.length} items):`,
      subs.map((s) => s.name),
    );
    return subs;
  }
  ...
  if (!cat) {
    console.warn(
      `[subcategories] "${slug}" — no matching category in API tree`,
    );
    return [];
  }
  ...
  console.log(
    `[subcategories] "${slug}" — from API (${subs.length} items):`,
    subs.map((s) => s.name),
  );
  return subs;
}
