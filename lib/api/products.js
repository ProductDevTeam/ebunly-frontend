...
const LOVED_URL = `${process.env.NEXT_PUBLIC_API_URL}/products/loved`;
...
/*
 * GET /products/loved — the five most purchased products platform-wide, already
 * sorted by purchase count. Public, no params.
 *
 * Revalidated every 5 minutes rather than hourly like the taxonomy: the ranking
 * moves with orders, and a homepage rail showing yesterday's bestsellers is the
 * one thing this endpoint exists to avoid.
 *
 * Returns [] if the call fails — the home page omits the section rather than
 * leaving five skeletons on screen forever.
 */
export const fetchLovedProducts = unstable_cache(
  async () => {
    const res = await fetch(LOVED_URL, {
      next: { revalidate: 300, tags: ["loved"] },
    });
    if (!res.ok) {
      console.error("[loved] Error", res.status, LOVED_URL);
      return [];
    }
    const json = await res.json();
    const items = Array.isArray(json.data)
      ? json.data
      : (json.data?.products ?? []);
    console.log(`[loved] OK — ${items.length} products`);
    return items.map(normalizeProduct);
  },
  ["products-loved"],
  { revalidate: 300, tags: ["loved"] },
);

function normalizeProduct(p) {
  // Cards render one dot per image and a "Personalizable" badge, so keep the
  // whole gallery and the personalisation flag — not just the first image.
...
export async function fetchProducts({
  ...
  if (occasionTags?.length) params.set("occasionTags", occasionTags.join(","));
...
function findSubcategory(taxonomy, typeName) {
...
export async function getSubFilters(typeName) {
  ...
  const sub = findSubcategory(taxonomy, typeName);
  if (sub) {
    const types = (sub.types ?? []).map((t) => t.name);
    console.log(`[taxonomy] sub-filters for "${typeName}":`, types);
    return types;
  }
...
export async function getSubFiltersForCategory(categoryName) {
  const taxonomy = await fetchTaxonomy();
...
export async function getTypePageDescription(typeName) {
  const taxonomy = await fetchTaxonomy();
  const sub = findSubcategory(taxonomy, typeName);
  return sub?.pageDescription ?? null;
}
