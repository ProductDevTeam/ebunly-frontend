...
| PDP: More gifts like this | pasted, not saved | section | components/shared/dashboard/product-detail/related.jsx | shoot.mjs "/products/<slug-with-relatedProducts>" --w 1440 --sel '[data-section="more-gifts"]' | done | n/a | replaces the old "Loved right now" band; real data via product.relatedProducts, which is empty on most current catalog items — verified the hide-when-empty path live, couldn't screenshot the populated state (no product in the sampled catalog currently has relatedProducts set) |
| PDP: Related to your search | pasted, not saved | section | components/shared/dashboard/product-detail/related-to-search.jsx | shoot.mjs "/products/personalized-cotton-t-shirt-cra-0005" --w 1440 --sel '[data-section="related-to-search"]' | done | n/a | mock data — confirmed no backend field/endpoint exists for search-context recommendations (checked live product payload + OpenAPI /search paths) |
